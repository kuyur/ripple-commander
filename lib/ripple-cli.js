  ...
  promise = promise.then(function(uuid) {
    var sendObject = {
      ...
      payment: {
        ...
        source_tag: opt_source_tag != null ? String(opt_source_tag) : '',
        source_amount: {
          value: parts[0],
          currency: parts[1],
          issuer: parts[2] || ''
        ...
        destination_tag: opt_destination_tag != null ? String(opt_destination_tag) : '',
        destination_amount: {
          value: parts[0],
          currency: parts[1],
          issuer: parts[2] || ''
        },
        invoice_id: opt_invoice_id != null ? String(opt_invoice_id) : '',
        paths: '[]',
...
RippleCLI.prototype.sendToBridge = function(destination, amount) {
  ...
  promise.then(function(response) {
    ...
    if (res.result === 'error') {
      ...
      var quote = res.quote;
      me.sendMoney(quote.destination_address, '' + quote.send.value + quote.send.currency + quote.send.issuer, null, quote.destination_tag, quote.invoice_id);
    }
  }, me.handleError_.bind(me));
};
