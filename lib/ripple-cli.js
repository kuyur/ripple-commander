...
var RippleIssuer = require('./ripple-issuer.js');
var RippleCode = require('./ripple-transaction-code.js').code;

function getIssuerName(address, no_empty) {
  if (no_empty) {
    return RippleIssuer.getIssuer(address) || address;
  } else {
    return RippleIssuer.getIssuer(address) || '';
  }
}
...
function formatNumber(number, digit) {
  return isNaN(number) ? '' : String(NumberUtils.round(number, NumberUtils.isInt(digit) ? digit : 8));
}
...
RippleCLI.prototype.start = function() {
  if (!this.account_ || !this.secret_) {
    console.log('Account or secret is not set.');
    process.exit(0);
  }
...
RippleCLI.prototype.handleReadline_ = function(line) {
  ...
  var params = line.trim().split(/\s+/);
  if (params[0] === 'new-wallet') {
    this.generateWallet();
  } else if (params[0] === 'get-balance') {
    this.getBalance();
  } else if (params[0] === 'get-trustlines') {
    this.getTrustLines()
  } else if (params[0] === 'grant-trustline') {
    this.grantTrustLine(params[1], params[2], params[3], params[4] === '--allow-rippling');
  } else if (params[0] === 'send') {
    var source_tag, destination_tag, invoice_id;
    for (var i = params.length - 1; i >= 3; --i) {
      var param = params[i];
    ...
    this.sendMoney(params[1], params[2], source_tag, destination_tag, invoice_id);
  } else if (params[0] === 'send-to-bridge') {
    this.sendToBridge.apply(this, params.slice(1));
  } else if (params[0] === 'get-payment') {
    this.retrievePayment.apply(this, params.slice(1));
  } else if (params[0] === 'get-payments') {
    this.retrievePayments();
  } else if (params[0] === 'get-orders') {
    this.getOrders();
  } else if (params[0] === 'place-order') {
    this.placeOrder.apply(this, params.slice(1));
  } else if (params[0] === 'cancel-order') {
    this.cancelOrder.apply(this, params.slice(1));
  } else if (params[0] === 'get-orderbook') {
    var limit;
    if (params[3] && StringUtils.startsWith(params[3], '--limit=')) {
      limit = Number(params[3].substring(8));
    }
    this.getOrderbook(params[1], params[2], limit);
  } else if (params[0] === 'get-transaction') {
    this.retrieveTransaction.apply(this, params.slice(1));
  } else if (params[0] === 'show-issuers') {
    var keyword;
    if (params[1] && StringUtils.startsWith(params[1], '--keyword=')) {
      keyword = params[1].substring(10);
    }
    this.showIssuers(keyword);
  } else if (params[0] === 'show-accounts') {
    this.listAccounts(params[1] === '--show-secret');
  } else if (params[0] === 'add-account') {
    this.addAccount.apply(this, params.slice(1));
  } else if (params[0] === 'change-account') {
    this.changeAccount.apply(this, params.slice(1));
  } else if (params[0] === 'remove-account') {
    this.removeAccount.apply(this, params.slice(1));
  } else if (params[0] === 'encrypt-wallet') {
    this.encryptWallet();
  } else if (params[0] === 'decrypt-wallet') {
    this.decryptWallet();
...
RippleCLI.prototype.getOrders = function() {
  ...
  PromiseAjax.get(url).then(function(response) {
    ...
    res.orders.forEach(function(obj, index) {
      ...
      var price = takerPay.value / takerGet.value;
      if (price >= 1) {
        price = NumberUtils.round(price, 5) + ' ' +  takerPay.currency + ' per ' + takerGet.currency;
      } else {
        price = NumberUtils.round(price, 8) + ' ' +  takerPay.currency + ' per ' + takerGet.currency;
      }
      if (obj.type === 'sell') {
        table.push([obj.sequence, obj.type, takerGet.value, takerGetCurrency, 'for',
          takerPay.value, takerPayCurrency, price]);
      } else {
        table.push([obj.sequence, obj.type, takerPay.value, takerPayCurrency, 'with',
          takerGet.value, takerGetCurrency, price]);
      }
    });
    console.log('ledger:' + res.ledger);
    console.log(table.toString());
...
RippleCLI.prototype.getOrderbook = function(currency1, currency2, limit) {
  this.pauseHandleReadline_();
  var url = StringUtils.formatString(API.GET_ORDER_BOOK.url, {
    'address': this.account_,
    'base': currency1,
    'counter': currency2
  });
  ...
  var me = this;
  PromiseAjax.get(url, {
    'limit': isNaN(limit) ? 10 : Math.max(10, Math.min(+limit, 400))
  }).then(function(response) {
    var res = response.getResponseJson();
    console.log('ledger: ' + res.ledger);
    var table = me.getTable_('orderbook');
    table.length = 0;
    var base = currency1.split('+')[0];
    var counter = currency2.split('+')[0];
    table.push([base, base, base + '/' + counter, base + '/' + counter, base, base]);
    var maxIndex = Math.max(res.bids.length, res.asks.length) - 1;
    var bidsTotal = 0, asksTotal = 0, bidSize = NaN, bidPrice = NaN, askSize = NaN, askPrice = NaN;
    var bid, ask;
    for (var i = 0; i <= maxIndex; ++i) {
      bid = res.bids[i];
      ask = res.asks[i];
      if (bid) {
        bidsTotal += Number(bid.taker_pays_total.value);
        bidSize = Number(bid.taker_pays_total.value);
        bidPrice = Number(bid.price.value);
      } else {
        bidPrice = NaN;
        bidsTotal = NaN;
        bidSize = NaN;
      ...
      if (ask) {
        asksTotal += Number(ask.taker_gets_total.value);
        askSize = Number(ask.taker_gets_total.value);
        askPrice = Number(ask.price.value);
      } else {
        askPrice = NaN;
        asksTotal = NaN;
        askSize = NaN;
      }
      table.push([formatNumber(bidsTotal), formatNumber(bidSize), formatNumber(bidPrice), formatNumber(askPrice), formatNumber(askSize), formatNumber(asksTotal)]);
    }
    console.log(table.toString());
    me.resumeHandleReadline_();
  }, me.handleError_.bind(me));
...
RippleCLI.prototype.showIssuers = function(keyword) {
  var issuers = RippleIssuer.getIssuersByName();
  var table = this.getTable_('issuer');
  table.length = 0;
  var key;
  for (key in issuers) {
    if (!StringUtils.isString(keyword) || key.toLowerCase().indexOf(keyword.toLowerCase()) >= 0) {
      table.push([key, issuers[key]]);
    }
  }
  console.log(table.toString());
};
  ...
  try {
    ...
    if (accounts.length === 0) {
      ...
      if (!address) {
        ...
        console.log(table.toString());
        promise = ReadlineUtils.question(this.readline_, 'Select the number (No.) to switch the activated account: ').then(function(answer) {
          answer = +answer;
...
RippleCLI.prototype.showHelp = function() {
  ...
  console.log('  get-orderbook <currency1+issuer1> <currency2+issuer2> [ --limit=<limit> ]')
  console.log('  get-transaction <hash>');
  console.log('  show-issuers [ --keyword=<issuer_name> ]');
