...
var code = require('./ripple-transaction-code.js');
...
RippleCLI.prototype.handleReadline_ = function(line) {
  ...
  if (line === 'new-wallet') {
    ...
    this.getBalance();
  } else if (line === 'get-trustline') {
    this.getTrustLine()
  } else if (line === 'get-order') {
    this.getOrder();
  } else if (line === 'help') {
    this.showHelp();
  } else if (line === 'exit') {
    this.exit();
  } else if (line.startsWith('grant-trustline')) {
    params = line.split(' ').slice(1);
    this.grantTrustLine.apply(this, params);
  } else if (line.startsWith('send-money')) {
    params = line.split(' ').slice(1);
    this.sendMoney.apply(this, params);
  } else if (line.startsWith('place-order')) {
    params = line.split(' ').slice(1);
    this.placeOrder.apply(this, params);
  } else if (line.startsWith('cancel-order')) {
    params = line.split(' ').slice(1);
    this.cancelOrder.apply(this, params);
  } else if (line.startsWith('retrieve-transaction')) {
    params = line.split(' ').slice(1);
...
RippleCLI.prototype.grantTrustLine = function() {
...
RippleCLI.prototype.placeOrder = function(type, currency1, amount1, currency2, amount2) {
  ...
  if (readlineSync.keyInYN(message)) {
    var url = AjaxUtils.formatString(API.PLACE_ORDER.url, {
      'address': this.account_
    });
...
RippleCLI.prototype.cancelOrder = function(sequence) {
  this.setPrompt_('');
  this.readline_.input.pause();
  var url = AjaxUtils.formatString(API.CANCEL_ORDER.url, {
    'address': this.account_,
    'sequence': sequence
  });
...
RippleCLI.prototype.retrieveTransaction = function(hash) {
  this.setPrompt_('');
  this.readline_.input.pause();
  var url = AjaxUtils.formatString(API.TRANSACTION.url, {
    'hash': hash
  });
