...
function loadExtraIssuers() {
  try {
    var data = fs.readFileSync('./issuers.json', 'binary');
...
exports.getIssuers = function(opt_address_prefix) {
  if (!issuers) {
    issuers = {};
    var key;
    for (key in trusted_issuers) {
      issuers[key] = trusted_issuers[key];
    }
    // load custom setting
    var extraIssuers = loadExtraIssuers();
    if (extraIssuers) {
      for (key in extraIssuers) {
        issuers[key] = extraIssuers[key];
      }
    }
  }
